...
class WOFComputerPlayer(WOFPlayer):
    ...
    def __init__(self, name_in, difficulty_in):
        WOFPlayer.__init__(self, name_in)
    ...
    def getPossibleLetters(self, guessed):
        ...
        for letter in LETTERS:
            if letter not in guessed:
                if letter not in VOWELS or self.prizeMoney > 250:
                    rv.append(letter)
        return rv
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if self.smartCoinFlip:
            for letter in reverse(self.SORTED_FREQUENCIES):
